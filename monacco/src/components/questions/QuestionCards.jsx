...
const QuestionGenerator = () => {
  ...
  const [mcqMarkc, setMcqMarkc] = useState(0);
  ...
  const handleOptionChange =(index,key,value , ans) => {
    if(key === ans){
      setMcqMark({
        ...mcqMark,
        [index]:true
  ...
  const handleDisplay = async () => {
    setMcqMarkc(0)
    for (let x in mcqMark) {
        if (mcqMark[x]) {
            setMcqMarkc(prevMcqMarkc => prevMcqMarkc + 1);
        }
    }
};

// Make sure to check the updated value after the loop
useEffect(() => {
    console.log(mcqMarkc);
}, [mcqMarkc]);
    ...
    <div className="p-4 max-w-xl mx-auto">
      ...
      {selectedQuestions.mcq.length > 0 && (
        <div className="mt-4">
                    ...
                    <li key={key} className="text-gray-700">
                      <label className="flex items-center">
                        <input
                          ...
                          onChange={() => handleOptionChange(index, key ,value , q.answer)}
                          className="mr-2"
                        />
          ...
          <Button onClick={handleDisplay} >summit</Button>
        </div>
      )}
           ...
           sx={{
            ...
            <ModalBody>
              ...
              <Flex>
                ...
                <div style={{ flex: "2" , marginTop:'-70px' }}>
                  {/* Monaco Editor */}
                  <Monaco
                      height="100px"
                      language="javascript"
                      ...
                      description={selectedQuestion.question} testcases={selectedQuestion.test_cases || []}
                  />
                </div>
              </Flex>
            </ModalBody>
